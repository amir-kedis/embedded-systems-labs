...
def record_data_point(ser, num_samples=AVG_MEAS, max_retries=RECONNECT_ATTEMPTS):
    ...
    while len(readings) < num_samples:
        if retries >= max_retries:
            print(
                f"[WARNING]: Maximum retries ({max_retries}) reached. Using available data."
            )
            break
...
def main():
    ...
    try:
        while position_count < MAX_MEAS:
            ...
            print(
                f"[INFO]: Reading: X={ax:.4f}, Y={ay:.4f}, Z={az:.4f}, Magnitude={magn:.4f}"
            )

            # Check if magnitude is close to 1g (expected for static accelerometer)
            if abs(magn - 1.0) > 0.15:  # Allow 15% deviation from 1g
                print(
                    "[WARNING]: Magnitude deviates significantly from 1g. Sensor may be moving or miscalibrated."
                )
        ...
        if all_data:
            ...
            print(
                f"  Min/Max magnitude: {np.min(magnitudes):.4f}g / {np.max(magnitudes):.4f}g"
            )
            ...
            if position_count < 8:
                ...
                print(
                    "[INFO]: It's recommended to collect data from at least 8 different orientations."
                )
        else:
